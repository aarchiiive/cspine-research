...
def log(project_name, preds, probs, labels, model_list, num_classes, class_names, mode="hard", use_wandb=False):
    ...
    print("- total")
    print("F1 score    : {}".format(metrics.f1_score(labels, preds)*100))
    ...
    if use_wandb:
        ...
        wandb.log({"f1 score" :  metrics.f1_score(labels, preds)*100})
...
def ensemble(weights_paths, 
    ...
    testloader = get_testloader("images/test/", "images/label/test.csv", batch_size=16)
    model_list = [pd.read_csv(os.path.join("weights", w, "hyperparameters.csv"))["model_name"][0] for w in weights_paths]
    models = [_models(m, num_classes=num_classes, drop_rate=0) for m in model_list] # model
    ...
    log(project_name, hard_preds, hard_probs, labels, model_list, num_classes, class_names, mode="hard")
    log(project_name, soft_preds, soft_probs, labels, model_list, num_classes, class_names, mode="soft")
