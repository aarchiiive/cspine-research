...
def log(project_name, weights_path, preds, probs, labels, model_name, num_classes, class_names, use_wandb=False):
    accuracy = metrics.accuracy_score(labels, preds) # 전체 정확도
    f1_score = metrics.f1_score(labels, preds, average=None) # 전체 f1 score
    confusion_matrix = metrics.multilabel_confusion_matrix(labels, preds, labels=range(num_classes)) # confusion matrix
...
if __name__ == "__main__":
    ...
    Args:
        weights_path (List): weights(.pt) 파일이 저장된 폴더이름
    ...
    test_model("lab01", class_names, project_name=project_name, use_wandb=True)
