...
def set_parameter_requires_grad(model, feature_extracting=True):
    """
    만약 requires_grad를 True로 설정하면 학습 중 모든 레이어에서 gradient가 계산되고
    False로 설정하면 output layer에서만 학습이 일어남
    """
...
def _models(model_name, 
    ...
    """
    torch library에서 모델들을 불러와서 imagenet으로 학습된 weight로 initialize (pretrained=True)
    drop_rate는 orverfitting을 방지하기 위해 fc layer 전에 추가함
    """
    if "resn" in model_name:
